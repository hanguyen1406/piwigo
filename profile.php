...
function encryptString($plaintext, $key, $iv) {
  ...
  $key = substr(hash('sha256', $key, true), 0, 32); // AES-256 key length is 32 bytes
  $iv = substr(hash('sha256', $iv, true), 0, 16);   // IV length is 16 bytes for AES-256-CBC
