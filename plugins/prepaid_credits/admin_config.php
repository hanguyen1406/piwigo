...
if (isset($_POST['save_config']))
{
  ...
  foreach (array_keys($sizes_options) as $size)
  {
    ...
    if($size == "original") {
      $price = $_POST["photo_cost"] * $_POST['size_'.$size];
      $query = "insert into piwigo_ppcredits_option
      VALUES ('Download(".$size.")', 'download.".$size."', ".$price.")
  ...
  //update option credit
  if($_POST['option'] != "---"){
    $method = explode(",", $_POST['option'])[0];
    $query = 'update piwigo_ppcredits_option set price = '.$_POST['price'].' WHERE method="'.$method.'";';
    print($query);
    pwg_query($query);
  }
...
$query = 'select name, method, price from piwigo_ppcredits_option';
