...
function attach_file_get_file($params, &$service)
{
  global $conf, $user;

  try {
    // Construct the SQL query
    $query = 'SELECT piwigo_images.file, piwigo_images.path, piwigo_images.id
              FROM piwigo_image_category
              INNER JOIN piwigo_images ON piwigo_image_category.image_id = piwigo_images.id
              WHERE piwigo_image_category.category_id = '.intval($params['album_id']);
    ...
    // Return the first file or the count of files
    if (count($files) > 0) {
      return $files; // Return the first file
    } else {
      return 'error';
    }
  } catch (\Throwable $th) {
...
function ppcredits_photo_editcredit($params, &$service)
{
  global $conf, $user;
  ...
  if($user['user_id'] == '1') {
    $value = $params['value'];
    $id = $params['id'];
    $res = pwg_query("update piwigo_user_infos set ppcredits = ".$value." where user_id = ".$id);
    if ($res) {
...
function ws_ppcredits_faceswap_download($params, &$service)
{
  global $conf, $user;

  $result = pwg_query("SELECT price FROM piwigo_ppcredits_option_price WHERE image_id=".$params['image_id']." and method='ppcredits.faceswap.download'");
  ...
  if($user['ppcredits'] >= $price) {
    try {
    $result = pwg_query("update piwigo_user_infos set ppcredits = ppcredits - ".$price." WHERE user_id = ".$user['id']);
      $row = pwg_db_fetch_assoc($result);
    } catch (\Throwable $th) {
      //throw $th;
    }
    $query = "select path from piwigo_images where id=".$params['image_id'];
    $result = pwg_query($query);
    $row = pwg_db_fetch_assoc($result);
    $path = $row['path']; 
    $path = substr($path, 2);

    return array('path' => $path, 'price' => $price);
  } else return "0";
  
}

function ws_ppcredits_faceswap_swap($params, &$service)
{
  global $conf, $user;

  $result = pwg_query("SELECT price FROM piwigo_ppcredits_option_price WHERE image_id=".$params['image_id']." and method='ppcredits.faceswap.swap'");
  $row = pwg_db_fetch_assoc($result);
  if($row){
    $price = $row['price'];  
...
;';
  ...
  if (!$image_credits)
  {
    ...
    $row = pwg_db_fetch_assoc($result);
    $image_credits = $row['price'];
    // $conf['ppcredits']['price_coefficient'][ $params['size'] ] * $conf['ppcredits']['photo_cost']
  } 
  
  // does the user has enough credits?
  if ($user['ppcredits'] < $image_credits)
  {
    return new PwgError(401, "Không đủ tiền");
  }
